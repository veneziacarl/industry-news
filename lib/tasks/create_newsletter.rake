        ...
        if feed.url.match(/feedburner/)
          if item.date == nil
            if item.dc_date >= date
              article = Article.find_or_create_by!(article_date: item.date, title: item.title, url: item.link, newsletter_feed: feed)
            end
          elsif item.date >= date
            article = Article.find_or_create_by!(article_date: item.date, title: item.title, url: item.link, description: item.description, newsletter_feed: feed)
          end
        elsif item.date >= date
          article = Article.find_or_create_by!(article_date: item.date, title: item.title, url: item.link, description: item.description, newsletter_feed: feed)
        end
